...
describe('OpenAI tool-loop context budgeting', () => {
  ...
  it('projects completed mutation evidence beside an exact-fit oversized task', () => {
    const model = 'openai:gpt-5.6-terra';
    const system = 'File the complete source-grounded artifact.';
    const tools: ChatToolDef[] = [{
      name: 'brain_put_page',
      description: 'Write one canonical page.',
      inputSchema: { type: 'object', properties: { slug: { type: 'string' } } },
    }];
    const sentence = 'The source records delivery milestones, participants, decisions, and evidence. ';
    const task = sentence.repeat(Math.ceil((220 * 1024) / sentence.length));
    const privateBody = `PRIVATE_MUTATION_BODY_${'x'.repeat(3_000)}`;
    const privateResult = `PRIVATE_MUTATION_RESULT_${'y'.repeat(500)}`;
    const messages: ChatMessage[] = [
      { role: 'user', content: task },
      {
        role: 'assistant',
    ...
    const compacted = compactToolLoopMessages(messages, budgets.byteSafeBytes, {
      mutatingToolNames: new Set(['brain_put_page']),
      preferredProjectionBytes: budgets.preferredProjectionBytes,
      preferredProjectionFits: candidate => openAiToolLoopRequestFits({
        budgets,
        system,
    ...
    expect(JSON.stringify(compacted)).not.toContain('PRIVATE_MUTATION_BODY_');
    expect(JSON.stringify(compacted)).not.toContain('PRIVATE_MUTATION_RESULT_');
    expect(openAiToolLoopRequestFits({
      budgets,
      system,
      tools,
  ...
  it('never restores mutation, mismatched-name, or failed singleton outputs', () => {
    ...
    for (const scenario of scenarios) {
      ...
      expect(preferredChecks).toBe(scenario.id === 'mismatch' ? 0 : 1);
    }
  });
});
