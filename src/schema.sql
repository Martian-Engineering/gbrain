...
-- blocks are durable. The JSONB envelope contains only static field paths and
-- value kinds; its array bound prevents this corrective surface becoming a
...
-- Finalized proposal authority outlives its disposable origin job. The public
-- proposal id initially equals the origin job id, while origin_job_id is only
-- an audit link and is nulled by ordinary job retention.
...
-- Exact pages and private optimistic baselines are authority-owned. They are
-- removed only by explicit authority expiry cleanup, never by job pruning.
...
-- Server-owned application state spans retries and apply jobs. These rows use
-- the durable proposal identity but intentionally do not reference a minion
-- job: pruning execution transcripts must not erase application proof.
...
-- One compact proof row per frozen timeline/link slot. Relation bodies stay in
-- proposal authority; this ledger retains only identities and replay proof.
CREATE TABLE IF NOT EXISTS ingestion_proposal_relation_outcomes (
  ...
  CONSTRAINT chk_ingestion_proposal_relation_digest
    CHECK (proposal_digest ~ '^[a-f0-9]{64}$'
...
-- composite unique index on the page tuple plus claim_hash.
